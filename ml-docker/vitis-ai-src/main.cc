...
/**
 * @brief Run DPU Task for Inception
 *
 * @return none
 */
void runInception(vart::Runner* runner_ori, const string imagePath) {
  ...
  int resizeHeight = static_cast<int>(inHeight / 0.875);
  int resizeWidth = static_cast<int>(inWidth / 0.875);
  ...
  for (unsigned int n = 0; n < images.size(); n += batchSize) {
        ...
        for (int w = 0; w < inWidth; w++) {
          for (int c = 0; c < 3; c++) {
            imageInputs[i * inSize + h * inWidth * 3 + w * 3 + 2 - c] =
                (image2.at<Vec3b>(h + offsetHeight, w + offsetWidth)[c] / 255.0 - 0.5) * 2;
    ...
    //   /* Display the impage */
    //   //cv::imshow("Classification of Inception_v1", imageList[i]);
    //   //cv::waitKey(10000);
    // }
  }
  ...
  std::cout<<"Total processed images "<<images.size()<<std::endl;
  std::cout<<"Inference throughput: "<<((float) images.size())/duration.count()<<" images/s"<<std::endl;
  std::cout<<"Inference latency: "<<duration.count()/((float) images.size())*1000.0 <<" ms"<<std::endl;
...
int main(int argc, char* argv[]) {
  // Check args
  if (argc != 3) {
    cout << "Usage of inception example: ./inception_example [model_file] [image_dir]" << endl;
    return -1;
  }
  auto graph = xir::Graph::deserialize(argv[1]);
  auto subgraph = get_dpu_subgraph(graph.get());
  CHECK_EQ(subgraph.size(), 1u)
      << "inception should have one and only one dpu subgraph.";
  ...
  /*run with batch*/
  runInception(runner.get(), argv[2]);
  return 0;
}
